...
CAMPOS = [
    ("codigo", "Código *", "Código já usado pelo cliente"),
...
class ProdutoDialog(ctk.CTkToplevel):
    def __init__(self, master, ao_salvar, produto=None):
        ...
        self.after(120, lambda: self.entries["codigo"].focus_set())
    ...
    def _montar(self) -> None:
        ...
        ctk.CTkButton(rodape, text="Cancelar", command=self.destroy, width=130,
        ...
        ctk.CTkButton(rodape, text="Salvar", command=self._salvar, width=150,
    ...
    def _salvar(self) -> None:
        ...
        if not codigo:
            return self._mostrar_erro("Informe o código do produto.")
        ...
        try:
            if self.produto:
                ...
                Produtos.update(self.produto.id, codigo=codigo, produto=nome, descricao=descricao,
                               ncm=ncm, **numericos)
            else:
                Produtos.create(codigo=codigo, produto=nome, descricao=descricao or None,
                                ncm=ncm or None, **numericos)
        except sqlite3.IntegrityError:
            return self._mostrar_erro(f"Já existe um produto com o código “{codigo}”.")
