...
class ProdutosPage(ctk.CTkFrame):
    ...
    def _montar(self) -> None:
        ...
        ctk.CTkButton(topo, text="+  Novo produto", command=self._novo, height=42, width=160,
                      corner_radius=tema.RAIO, font=tema.FONTE_SUBTITULO,
        ...
        self.busca = ctk.CTkEntry(barra, placeholder_text="Buscar por nome ou id…", height=40,
                                  font=tema.FONTE_CORPO, border_width=0, fg_color="transparent")
        self.busca.grid(row=0, column=0, sticky="ew", padx=(16, 8), pady=6)
        self.busca.bind("<KeyRelease>", lambda _e: self.recarregar())

        ctk.CTkButton(barra, text="Editar", command=self._editar, width=90, height=32,
                      corner_radius=8, font=tema.FONTE_CORPO, fg_color=tema.NEUTRO,
                      hover_color=tema.NEUTRO_HOVER, text_color=tema.TEXTO).grid(row=0, column=1, padx=4)
        ctk.CTkButton(barra, text="Excluir", command=self._excluir, width=90, height=32,
                      corner_radius=8, font=tema.FONTE_CORPO, fg_color="transparent",
    ...
    def recarregar(self) -> None:
        ...
        self.tabela.preencher([
            (
                p.id, p.id, p.produto, p.ncm or "—", p.cx or 0,
